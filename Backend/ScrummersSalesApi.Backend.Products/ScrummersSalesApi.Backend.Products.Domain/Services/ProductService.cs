...
namespace ScrummersSalesApi.Backend.Products.Domain.Services
{
    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<IEnumerable<ProductDto>> GetProductsByIdsAsync(IReadOnlyList<Guid> ids)
        {
            if (ids == null || ids.Count == 0)
                return Enumerable.Empty<ProductDto>();

            // Dapper query to retrieve multiple products by Id
            // Includes Stock column
            const string sql = @"
                                SELECT
                                    Id,
                                    Name,
